...
- `backend/apps/api/src/main.ts`：对外 API（端口如 6100）
...
| REST API | 访问 `http://localhost:6100/users` 返回 JSON | 浏览器/curl |
...
- 后端 API 服务启动在 `http://localhost:6100`
- 前端启动在 `http://localhost:5173`（Vite 默认端口，按实际为准）
- 浏览器访问 `http://localhost:6100/users` 能返回数据
- 前端页面能成功请求并展示用户列表
